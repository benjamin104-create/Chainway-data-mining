...
def penalty(n_refs: int, *, per_ref: float = 0.0) -> float:
    ...
    **量過了，先維持 0。** 改成一款存所有顏色之後，顏色數變得很不平均
    （真實的款有 1～10 個顏色），所以特地測了不平均的情況：60 款、
    一款 1～10 色、共 314 張參考圖、50 題。

        per_ref = 0.0   Top-1 98.0%
        per_ref = 1.0   Top-1 98.0%

    答錯的那一題，搶走第一名的款有 8 個顏色（全庫平均 5.2），方向上
    確實是「顏色多的佔便宜」—— 但 50 題只錯 1 題，這個偏差小到量不出
    代價。所以維持 0，等真實資料上有足夠的錯例再回來看。
