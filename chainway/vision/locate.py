...
# 主體最多能佔畫面多少。超過就沒有外輪廓可言，領口／袖長／衣長全部量不得。
#
# 這一條是拿使用者自己的一張穿搭照踩出來的。那張是模特兒穿著藏青上衣、
# 米白裙、拿棕色包的翻拍照。切成上下三段之後，中段**通過了背景那兩道檢查**
# （她背後是素色牆，邊緣一圈確實乾淨），然後量出：
#
#     領型 大開領（實際是圓領）
#     衣長 長版／長罩衫（它量的是整個人，不是那件上衣）
#     圖案在下擺 11.6%（實際的設計重點是胸前的格紋胸飾，
#                        它抓到的是底下那個棕色包）
#     主色 #C7CBC9 無色（那件是藏青，它平均掉了白牆與米白裙）
#
# 四項錯三項，沒有任何警告。而 fill = 0.98 一眼就說明了原因：
# 畫面裡沒有「衣服以外」的東西，量測靠的邊界根本不存在。
#
# 門檻取 0.80。實測該量的圖落在 0.23–0.47（去背商品照本來就有留白），
# 不該量的落在 0.92–0.98（布樣、穿搭照裁切），中間空了一大段。
# 這個數字和 sketch/from_photo.py 是同一個 —— 那邊先發現、這邊沒補，
# 同一個洞補了一邊漏了另一邊，所以現在定義收在這裡一份。
FILL_MAX = 0.80
...
def is_garment_shot(img) -> tuple[bool, str]:
    ...
    if s["fill"] > FILL_MAX:
        return False, (f"主體佔了畫面 {s['fill']:.0%}，看不到衣服的外輪廓 —— "
                       f"看起來是穿在人身上的照片、滿版平拍或布料，"
                       f"領口與袖長沒有邊界可以量")
    if s["aspect"] < 1.0:
...
def locate(img, category: str | None = None, *, top: int = 3,
    ...
    if not real:
        # 措辭很重要。原本寫「整件素色，沒有偵測到明顯的局部設計」——
        # 那句話會被讀成「這件沒有設計重點」，而那是錯的，也是危險的。
        #
        # 這支程式找的是「跟衣身**顏色不一樣**的區塊」。同色的結構設計
        # —— 荷葉、剪接、褶、假兩件、蝴蝶結、緄邊 —— 它一個都看不到。
        # 拿貴司 144 個暢銷款的品名量過：**31% 的款只有同色結構、沒有
        # 異色圖案**，這些款全部會落到這一支，全部被記成「素色」。
        #
        # 使用者傳過一件實例：藏青針織上衣，主設計是胸前一整片同色荷葉，
        # 這裡只會看到別在胸口的那個格紋緞帶結 —— 而那還可能不算衣服。
        #
        # 所以這裡回的是「沒有偵測到異色圖案」，不是「沒有設計」。
        # 兩者在後面的統計裡意思完全不同：前者是量測的邊界，
        # 後者是關於這件衣服的結論，而這支程式下不了那個結論。
        note = ("沒有偵測到異色圖案。注意：同色的結構設計（荷葉、剪接、褶、"
                "假兩件、蝴蝶結）本程式看不到，不代表這件衣服沒有設計重點"
                if not blobs else
                "只偵測到疑似吊架／吊牌。同色的結構設計本程式看不到，"
                "不代表衣服本身沒有設計重點")
        return {"裝飾": [], "描述": note, "無異色圖案": True,
