...
def run(cfg: Config | None = None, *, limit: int | None = None,
    ...
    for i, p in enumerate(files, start=1):
        ...
        try:
            ...
            continue
        if not res["裝飾"]:
            # 三種都要分開記，混在一起後面的分母就錯了：
            #   非衣物      這張圖不該問位置（布樣、版面、穿搭照）
            #   無異色圖案  問了，答案是「沒有跟衣身顏色不同的區塊」
            #
            # 第二個**不叫素色**。這支程式只看得到異色的東西；同色的荷葉、
            # 剪接、褶、假兩件它一個都看不到，而貴司 144 個暢銷款裡有 31%
            # 正是只有同色結構、沒有異色圖案的款。叫它「素色」等於把
            # 「我看不到」寫成「它沒有」，而那三成款會被靜靜地歸錯類。
            zone = "非衣物" if res.get("非衣物") else "無異色圖案"
