...
REPORTS: list[tuple[str, str, str, str, int]] = [
    ...
    ("商品規劃展開圖.html", "商品規劃展開圖",
     "各品類各季該開幾款 —— 用過去五年那一格真的賣掉多少來回答。", "6", 90),
    ("reports/*.html", "季別診斷報告",
     "每一季的完銷率、袖長對照、上架期重疊、銷冠。", "7", 30),
    ...
    ("影像比對稽核/*.csv", "影像比對稽核",
     "為什麼有些款找不到系統圖 —— 分成五類，每類給實例。", "5", 30),
    ("重複款偵測.html", "重複款偵測",
     "這一季要開的是不是三年前做過了 —— 分成「一直好（該延續）」與"
     "「愈來愈差（該停）」，兩件照片並排讓人自己確認。", "8", 30),
    ("客戶意見調查.html", "客戶意見調查",
     "LINE 上傳發票後回填的表單 —— 流程設計、客人會看到的樣子、"
     "以及貼進 LINE／Google 表單的題目稿。", "4", 90),
    ("sketches/*/spec.md", "市調圖線稿與規格",
     "市調照片 → 裁切、線稿、SVG、平塗彩現、色票，加一份款式規格與"
     "繪圖 prompt。規格能量的自動量，量不到的明說要人填。",
     "sketch --image 檔名 --auto", 90),
    ("版型量測覆核.html", "版型量測　覆核",
     "★ 要看：線畫得對不對。把量到的肩線、領口、袖口、身寬畫回原圖上。"
     "版型量錯了也會給出一個看起來正常的標籤，沒有警告 —— 只能用看的。",
     "silhouette --sheet", 30),
    ("版型銷售關係/*.csv", "版型 × 銷售",
     "領型／袖長／衣長跟賣不賣得動的關係，附 bootstrap 區間。"
     "先驗量測準不準（拿品名當答案），驗不過的屬性不給比較。",
     "locate 之後跑 silhouette", 60),
    ("庫存對不上的款.csv", "庫存對不上的款",
    ...
    ("motif/*.csv", "熊／格紋拆解",
     "圖案的位置、比例、形式與售罄率的關係（附信賴區間）。"
     "位置與形式是從品名讀的，不是影像認的 —— 品名寫得比模型猜得準。",
     "motif", 60),
]
