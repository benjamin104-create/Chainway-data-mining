...
rem ===================================================================
rem  Kinloch Anderson - counter calibration - was their call right?
rem
rem  Double-click this file after the counter feedback has been pasted
rem  into data\feedback\sales_feedback.csv. It compares what each
rem  person said at the time against what actually sold, and scores
rem  both their hit rate and whether "very sure" really meant more
rem  accurate.
rem    1. downloads the latest code from GitHub (no git needed)
rem    2. creates / reuses the Python environment
rem    3. rebuilds the tables, then scores the feedback
rem
...
rem ===================================================================
cd /d "%~dp0"
title Kinloch Anderson - counter calibration
...
echo.
echo  ==================================================================
echo   Step 3 of 3  -  scoring the feedback
echo  ==================================================================
rem The master table has to be current, otherwise styles that sold out
rem since the last run will not be scored.
...
echo.
echo   Done. The numbers above are also saved as CSV in:
echo     %OUT%
echo.
echo   How to read it: Brier is lower-is-better. Someone whose hit rate
echo   is high AND whose "very sure" answers beat their unsure ones is
echo   worth listening to early in a season.
echo.
pause
exit /b 0
